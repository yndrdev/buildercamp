...
import { CheckCircle, User, Briefcase, Stack, ChatDots, EnvelopeSimple } from '@phosphor-icons/react'
  ...
  return (
    <Sidebar className="border-r border-[var(--border)]">
      <SidebarHeader className="p-5 pb-4">
        ...
        {isComplete ? (
          /* Completion callout — replaces progress ring once intake is done */
          <div className="p-3.5 rounded-[var(--radius-lg)] bg-[var(--coral)]/8 border border-[var(--coral)]/30">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle weight="fill" className="w-[16px] h-[16px] text-[var(--coral)] shrink-0" />
              <div className="text-[12px] font-semibold text-[var(--text-primary)]">You&apos;re all set</div>
            </div>
            <p className="text-[11px] text-[var(--text-muted)] leading-relaxed flex items-start gap-1.5">
              <EnvelopeSimple weight="regular" className="w-[12px] h-[12px] mt-0.5 shrink-0 text-[var(--text-muted)]" />
              <span>Confirmation sent to your inbox. Your team contact will follow up with session details.</span>
            </p>
          </div>
        ) : (
          /* Progress ring */
      ...
      <SidebarContent className="px-2">
        {/* Meta: Name, Role, Session — non-interactive checklist */}
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <ul className="flex flex-col gap-0.5 px-1">
              {[
                ...
                <li
                  key={i}
                  aria-label={`${item.label}${item.done ? ' (complete)' : item.active ? ' (current step)' : ' (pending)'}`}
                  className="flex items-center gap-3 py-2.5 px-2 rounded-[var(--radius-md)] select-none"
                >
                  {item.done ? (
                    <CheckCircle weight="fill" className="w-[16px] h-[16px] text-[var(--coral)] shrink-0" />
                  ) : (
        ...
        {/* Question sections — non-interactive progress checklist */}
                ...
                <ul className="flex flex-col gap-0.5 px-1">
                  {section.questions.map((q) => {
                    ...
                    return (
                      <li
                        key={q.id}
                        aria-label={`${q.label}${isAnswered ? ' (answered)' : ' (pending)'}`}
                        className="flex items-center gap-3 py-2 px-2 rounded-[var(--radius-sm)] select-none"
                      >
                        {isAnswered ? (
